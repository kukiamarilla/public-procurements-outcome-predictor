...
% Formatting follows the IEEE conference \LaTeX{} template (see paper/vendor/IEEE-conference-template-062824) with the matching \texttt{IEEEtran.cls}.
...
\title{Lexical and LLM-Based Representations for Public Tender Outcome Prediction: A Comparative Study}
\author{\IEEEauthorblockN{Isaac Gabriel Amarilla Benitez}
\IEEEauthorblockA{\textit{Universidad Nacional de Asunci\'on}\\
Asunci\'on, Paraguay\\
sc.amarilla@fpuna.edu.py}
\and
\IEEEauthorblockN{Luis Fernando Caballero Ramoa}
\IEEEauthorblockA{\textit{Universidad Nacional de Asunci\'on}\\
Asunci\'on, Paraguay\\
luisfercaballero27@fpuna.edu.py}
\and
\IEEEauthorblockN{Julio Manuel Paciello Coronel}
\IEEEauthorblockA{\textit{Universidad Nacional de Asunci\'on}\\
Asunci\'on, Paraguay\\
...
\makebox[\textwidth][c]{%
\begin{tabular}{c}
{\sublargesize Juan Ignacio Pane Fernandez}\\[0.3ex]
{\normalsize\textit{Universidad Nacional de Asunci\'on}}\\
{\normalsize Asunci\'on, Paraguay}\\
{\normalsize jpane@pol.una.py}
\end{tabular}}
}
...
\begin{abstract}
<<<<<<< HEAD
This short paper compares lexical and LLM-based representations for predicting public tender outcomes from bidding documents. Using a processed subset of 1{,}162 road-construction procurements from Paraguay's Dirección Nacional de Contrataciones Públicas, we evaluate dummy baselines, TF--IDF with logistic regression, mean-pooled LLM embeddings, and a lightweight transformer over LLM chunk embeddings under deterministic stratified 5-fold cross-validation. TF--IDF with logistic regression provides the strongest ranking performance, reaching mean ROC AUC 0.814 and mean PR AUC 0.899. The cross-chunk transformer does not improve ranking over this lexical baseline, but achieves the best calibration, with mean Brier score 0.162, and shows the greatest stability under threshold tuning. The results indicate that bidding documents contain useful ex ante predictive signal and that ranking, calibration, and threshold behavior should be evaluated separately in public procurement monitoring.
\end{abstract}
...
This paper studies that question through a direct comparison between sparse lexical features and LLM-based dense representations. The analysis focuses on road construction tenders, whose bidding documents are often relatively structured and standardized. The task is relevant to electronic government because early risk signals can support procurement monitoring and analytical decision support. Sparse lexical models remain strong and interpretable baselines in document classification \cite{wang2012baselines,joulin2017bag}, while pretrained language models offer flexible dense representations for long and heterogeneous text \cite{yang2016han,beltagy2020longformer,zaheer2020bigbird}. In the present setting, the goal is to understand how much predictive signal can be recovered from lexical cues, how much is already captured in frozen dense embeddings, and whether explicit cross-chunk modeling provides additional value.
...
\begin{IEEEkeywords}
public procurement, eGovernment, eDemocracy, tender documents, large language models, outcome prediction
\end{IEEEkeywords}

\section{Introduction}
Public procurement is a core eGovernment process: it translates public budgets into contracts, works, and services through increasingly digital platforms. It is also relevant to eDemocracy because open procurement records expand the capacity of oversight bodies, journalists, firms, and citizens to scrutinize how public resources are allocated. These capabilities align with digital-government agendas on open data and accountable analytics for public-sector oversight. Recent public-sector work frames procurement analytics as a tool for risk-based oversight \cite{salazar2024vigia}. Bidding documents are central to this setting because they define administrative conditions, technical requirements, qualification rules, and delivery constraints before the final outcome is known. Recent procurement-domain evidence also shows that call descriptions can predict competition and cost-effectiveness outcomes \cite{acikalin2024procurementcalls}. The relevant question is therefore not only whether tender outcomes can be predicted from document text, but also which representation of that text is most informative for accountable public-procurement monitoring.

This paper studies that question through a direct comparison between sparse lexical features and LLM-based dense representations. The analysis focuses on road construction tenders, whose bidding documents are often relatively structured and standardized. Sparse lexical models remain strong and interpretable baselines in document classification \cite{wang2012baselines}, and recent long-document work confirms that lexical or hybrid representations can remain competitive rather than being uniformly dominated by transformer architectures \cite{alvaprincipe2025survey}. At the same time, embedding research has expanded toward LLM-based dense representations \cite{wang2024improvingembeddings}. In the present setting, the goal is not to establish a universally superior model family, but to understand how much predictive signal can be recovered from lexical cues, how much is already captured in frozen dense embeddings, and whether explicit cross-chunk modeling provides additional value.
...
<<<<<<< HEAD
Each PBC document is first converted from PDF to text by the upstream extraction pipeline. The resulting text is tokenized and split into overlapping chunks. In the current implementation, the chunk length is 4096 tokens with a stride of 2048 tokens. A pretrained causal language model, \texttt{openai/gpt-oss-20b}, is used as a frozen document encoder. It was selected as a high-capacity general-purpose encoder that avoids task-specific fine-tuning and keeps the comparison focused on document representations and downstream aggregation under limited computational resources. Chunks are processed in micro-batches to reduce GPU memory pressure.
=======
Each PBC document is first converted from PDF to text by the upstream extraction pipeline. The resulting text is tokenized and split into overlapping chunks. In the current implementation, the chunk length is 4096 tokens with a stride of 2048 tokens. A pretrained causal language model, \texttt{openai/gpt-oss-20b}, is used as a frozen document encoder \cite{agarwal2025gptoss}. This model was selected as a practical open-weight LLM for extracting dense representations from Spanish procurement text while keeping the experimental pipeline reproducible. Freezing the encoder also avoids fine-tuning a large model on a modest dataset and makes the comparison focus on how much signal is available in the pretrained representations and how it is aggregated downstream, consistent with recent work on LLM-based text embeddings \cite{wang2024improvingembeddings}. Chunks are processed in micro-batches to reduce GPU memory pressure.
>>>>>>> paper-camera-ready-from-submission
...
=======
Second, we use a lexical baseline based on TF--IDF features with up to 50{,}000 unigram and bigram features, minimum document frequency $5$, and a logistic-regression classifier with balanced class weights. We use TF--IDF rather than static dense embeddings such as Word2Vec because it provides a strong, interpretable bag-of-words reference that remains competitive in long-document settings \cite{wang2012baselines,alvaprincipe2025survey} and isolates the comparison against frozen LLM representations without adding another trainable embedding stage on limited supervision. Linear bag-of-words baselines of this kind are standard references in text classification \cite{wang2012baselines}, and recent long-document evidence suggests that lexical representations remain competitive in some settings \cite{alvaprincipe2025survey}.
>>>>>>> paper-camera-ready-from-submission
...
The supervised subset used for modeling is smaller because each procurement must have an available PBC, successful text extraction, precomputed embeddings, and a status belonging to the final binary target classes. After applying these constraints, the effective dataset contains 1{,}162 examples. The collection process was automated rather than manually curated, and the resulting subset was shaped by document availability, extraction success, embedding-generation feasibility, and processing constraints rather than by a sampling design targeted at representativeness. The conclusions therefore apply to the processed subset rather than automatically to the full procurement population.
=======
...
\section{Experimental Setup}
\subsection{Dataset}
The metadata snapshot used in this paper was constructed from public procurement records published by the Dirección Nacional de Contrataciones Públicas (DNCP) of Paraguay and retrieved through the official open-data API endpoint \texttt{v3/doc/search/processes}. The initial pool contains 10{,}628 road-construction procurements under catalog code \texttt{72131701}: 10{,}000 completed processes materialized from a previous DNCP scraping pipeline and 628 additional non-successful processes collected by status. To reduce class imbalance, retrieval and processing prioritized non-successful procurements before completed ones.
...
For each fold, the model is trained on four partitions and evaluated on the held-out partition; reported performance is therefore cross-validated rather than based on a single fixed test set. The paper figures use out-of-fold predictions, so every plotted prediction corresponds to a procurement held out from the model that produced it. In a second analysis, threshold sensitivity is examined with a nested procedure: inside each outer training fold, an inner validation split selects the F1-maximizing threshold, which is then evaluated on the held-out outer fold.
=======
...
=======
Metric choice under imbalance depends on the operational objective: PR-based summaries emphasize positive-class retrieval, while ROC AUC and PR AUC respond differently to prevalence and should be interpreted in context \cite{richardson2024roc}. Because PR AUC can be misleading when the positive class is frequent, it is interpreted together with the positive prevalence $\pi$ in the held-out cross-validation folds. In the current implementation, the system therefore also logs \texttt{positive\_prevalence} and \texttt{pr\_auc\_excess\_over\_prevalence}, where the latter is defined as PR AUC minus $\pi$. This quantity provides a compact estimate of how much the model improves over the positive-class frequency as a reference level commonly used when discussing no-skill PR behavior in imbalanced settings.
>>>>>>> paper-camera-ready-from-submission
...
=======
Calibration tells a different story from ranking. The cross-chunk transformer achieves the best Brier score ($0.162$), followed by the mean-pooled MLP ($0.165$), while TF--IDF plus logistic regression is weaker ($0.179$) and mean-pooled logistic regression is substantially worse ($0.201$). This pattern indicates that lexical features can be highly effective for ranking examples, but that LLM-based dense representations provide more reliable probabilities. Fig.~\ref{fig:calibration} is consistent with this interpretation: among the three most informative model families, the transformer remains closest to the diagonal across much of the probability range, while TF--IDF shows stronger ranking than calibration. This matters because model selection in public-procurement monitoring may depend not only on ranking but also on whether predicted probabilities can support downstream triage or risk scoring \cite{silvafilho2023calibration}.
>>>>>>> paper-camera-ready-from-submission
...
\section{Limitations}
Several limitations should qualify the interpretation of these results. First, the supervised dataset is a processed subset rather than a representative sample of the full procurement population. Inclusion required document availability, successful text extraction, successful embedding generation, and one of the two final target statuses. These requirements may exclude procurements in non-random ways, so the conclusions should be understood as applying to the processed subset.
=======
Overall, the comparison is informative even without a single dominant winner. For eGovernment and eDemocracy applications, this is precisely the point: procurement monitoring systems require more than a high ranking score. They also need calibrated probabilities, transparent baselines, and thresholds whose behavior can be explained before they are used to prioritize human attention. The results show that lexical models are very competitive in this domain, that frozen LLM embeddings already capture much of the useful dense signal, and that the principal comparative advantage of the transformer lies in calibration and threshold stability rather than in raw ranking performance.
