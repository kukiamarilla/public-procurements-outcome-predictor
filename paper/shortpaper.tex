...
\begin{abstract}
This short paper compares lexical and LLM-based representations for predicting public tender outcomes from bidding documents. Using procurement data published by Paraguay's Dirección Nacional de Contrataciones Públicas, we evaluate dummy baselines, TF--IDF with logistic regression, mean-pooled LLM embeddings with logistic regression or a small MLP, and a lightweight transformer over LLM chunk embeddings. Labels are derived from final procurement status, mapping \texttt{complete} to successful outcome and \texttt{unsuccessful}/\texttt{cancelled}/\texttt{canceled} to non-successful outcome.

The experiments use a processed supervised subset of 1{,}162 road-construction procurements under deterministic stratified 5-fold cross-validation. TF--IDF with logistic regression provides the strongest ranking performance, reaching mean ROC AUC 0.814 and mean PR AUC 0.899. The cross-chunk transformer does not improve ranking over the strongest lexical baseline, but it achieves the best calibration, with mean Brier score 0.162, and shows the greatest stability under threshold tuning. The results indicate that bidding documents contain useful ex ante predictive signal, but that ranking, calibration, and threshold behavior should be evaluated separately when considering AI-based support tools for public procurement monitoring.
\end{abstract}
