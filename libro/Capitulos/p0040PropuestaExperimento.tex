...
La generación de embeddings se ejecutó en una instancia de cómputo acelerado en la nube, provista por RunPod y equipada con una GPU NVIDIA H200 compatible con CUDA. El proveedor se consigna como información de trazabilidad, pero no constituye un requisito de reproducción: una infraestructura equivalente puede utilizarse si ofrece los recursos y el entorno de software necesarios. El código selecciona una GPU cuando está disponible, utiliza \texttt{bfloat16} en ese caso y conserva los tokens en CPU hasta transferir cada microlote. La evidencia de ejecución registra 671 fallos \texttt{OutOfMemoryError}. La variante y la memoria exacta de la GPU, así como el modelo de procesador y la memoria RAM de la instancia, no se encuentran preservados en los artefactos disponibles; por ello, no se atribuyen especificaciones adicionales no verificables.
...
\midrule
Acelerador & GPU NVIDIA H200 compatible con CUDA \\
Entorno de ejecución & Instancia de cómputo acelerado en la nube provista por RunPod \\
...
Configuración restante & Variante y memoria exacta de la GPU, procesador y memoria RAM no preservados; deben registrarse en una reproducción \\
...
\input{Capitulos/shortpaper_fragments/configuracion_experimental}
...
Los artefactos públicos archivados en Zenodo incluyen el código y los 1.162 archivos \texttt{.pt} de embeddings congelados empleados en los experimentos (doi:10.5281/zenodo.20128768). El paquete de embeddings incorpora un manifiesto con el identificador de cada archivo, sus dimensiones y su suma de verificación SHA--256, además de metadatos agregados y documentación de uso. Esto permite reproducir el entrenamiento y la evaluación de los modelos basados en LLM sin volver a ejecutar la costosa etapa de codificación documental.

El depósito excluye credenciales y los datos fuente locales. Por tanto, reproducir los experimentos a partir de los embeddings publicados no requiere acceso a la infraestructura original, pero reconstruir toda la canalización desde los PDF exige obtener legítimamente los documentos desde la DNCP y disponer del modelo base. Esta distinción delimita el grado de reproducibilidad alcanzado.
